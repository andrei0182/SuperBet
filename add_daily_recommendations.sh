#!/bin/bash
# add_daily_recommendations.sh — [SuperBet repo] adds the daily
# recommendation script: cross-references BetExplorer's "100% Over 2.5"
# sheet (real historical hit-rate data) against SuperBet's own daily
# "Matches" sheet (live odds, availability on the platform), matches by
# team name, builds a case-study email, and sends it via Gmail SMTP.
# Run from the SuperBet repo root: bash add_daily_recommendations.sh
set -e
...
cat > .github/workflows/daily-recommendations.yml << 'EOF'
name: Daily 100% Over 2.5 recommendations
...
jobs:
  send-recommendations:
    ...
    steps:
      ...
      - name: Checkout BetExplorer
        uses: actions/checkout@v4
        with:
          repository: andrei0182/bet
          ref: claude/trusting-ramanujan-2xdf01
          path: bet
      ...
      - name: Install BetExplorer dependencies
        working-directory: bet
        run: pip install -r requirements.txt

      - name: Install SuperBet dependencies
        working-directory: SuperBet
        run: pip install -r requirements.txt

      - name: Compute today's date
        id: date
        run: echo "value=$(date +%F)" >> "$GITHUB_OUTPUT"

      - name: Run BetExplorer scraper (with stats)
        working-directory: bet
        run: python main.py --date ${{ steps.date.outputs.value }} --with-stats

      - name: Run SuperBet scraper (all leagues)
        working-directory: SuperBet
        run: python main.py --date ${{ steps.date.outputs.value }} --all
...
python -m py_compile daily_recommendations.py && echo "OK — syntax valid."
