#!/bin/bash
# diagnose_matching.sh — [SuperBet repo] one-off diagnostic: prints the 9
# BetExplorer "100% Over 2.5" team pairs alongside a sample of Superbet's
# normalized team names, so we can see whether the 0-match result is
# genuine (no overlap in today's leagues) or a name-formatting mismatch.
# Run from the SuperBet repo root, after both matches.xlsx files exist:
#   python diagnose_matching.py --bet-xlsx ../bet/output/matches.xlsx --superbet-xlsx output/matches.xlsx
set -e

cat > diagnose_matching.py << 'EOF'
...
echo "diagnose_matching.py created. Run it with:"
echo "  python diagnose_matching.py --bet-xlsx ../bet/output/matches.xlsx --superbet-xlsx output/matches.xlsx"
